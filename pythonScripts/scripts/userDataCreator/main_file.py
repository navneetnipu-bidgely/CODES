...
def create_user_enrollment_record(user_configs):
    ...
    for id in range(1,users_to_be_created+1):
...
def create_meter_enrollment_record(meter_configs,meters_to_be_created):
    ...
    for id in range(1,meters_to_be_created+1):
...
def create_raw_data_for_a_user(raw_data_configs,id):
    ...
    if raw_data_end_timestamp==0:
        raw_data_end_timestamp=get_current_timestamp()
    ...
    raw_data_record=populate_variable_fields_with_static_key(dummy_raw_data_record,variable_fields_with_static_key,id)
    epoch_timestamp=raw_data_start_timestamp
    while epoch_timestamp <= raw_data_end_timestamp:
        consumption=get_random_usage_data()
        ...
        single_raw_data_record_for_a_time=populate_consumption_and_date_time_into_raw_data_record(raw_data_record,consumption,date_time)
...
def create_billing_data_for_a_user(billing_data_configs,user_raw_data_consumption_map,billing_cycle_list,rate,id):
    ...
    billing_data_record=populate_variable_fields_with_static_key(dummy_billing_data_record,variable_fields_with_static_key,id)
    ...
    for data in aggregated_raw_data_per_bc:
        ...
        single_billing_data_record=populate_billing_data_cost_consumption_values(billing_data_record,cost,consumption,bc_start,bc_end,bc_duration)
...
def aggregate_raw_data(user_raw_data_consumption_map,agg_mode,billing_cycle_list,is_billing_end_date_inclusive):
    ...
    if agg_mode=="DAY":
        ...
        if len(billing_cycle_list)>0:
            aggregated_data=aggregate_raw_data_on_bc(user_raw_data_consumption_map,billing_cycle_list,is_billing_end_date_inclusive)
...
def aggregate_raw_data_on_bc(user_raw_data_consumption_map,billing_cycle_list,is_billing_end_date_inclusive):
    ...
    for billing_cycle in billing_cycle_list:
        ...
        consumption_for_this_bc=extract_consumption_data_in_window(user_raw_data_consumption_map,bc_start_timestamp,bc_end_timestamp)
...
if __name__=='__main__':
    ...
    if create_meters:
        ...
        create_meter_enrollment_record(meter_configs,meters_to_be_created)
    ...
    if create_raw_data and create_billing_data:
        ...
        for id in range(1,unique_users_count+1):
            ...
            create_billing_data_for_a_user(billing_data_configs, user_raw_data_consumption_map, billing_cycle_list, rate,
                                           id)

    elif create_raw_data:
        print("creating raw data files...")
        raw_data_configs = configs["raw_data"]
        create_raw_data_for_a_user(raw_data_configs, id)
